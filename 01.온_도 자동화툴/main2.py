...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # ✅ login QLabel(objectName="code") 스타일 적용하려면
        #    Qt Designer에서 "코드" 라벨의 objectName을 "code"로 바꿔줘야 함.
        #    (지금 이미 code라고 가정)
    ...
    def _lock_ui_for_license_check(self, lock: bool, hint: str = "확인 중..."):
        ...
        for name in [
            ...
            "lotto_upload_btn", "lotte_upload_btn",  # 혹시 오타 대비
    ...
    def ensure_license_then(self, callback, ttl_seconds: int = 3600, retry_message: str = None):
        """
        ...
        # 2) 만료면 서버 재검증 (사용자에게는 "확인 중..."만 보이게)
        self._lock_ui_for_license_check(True, "확인 중...")

        # 최근 입력 코드가 있으면 그걸 사용, 없으면 현재 입력칸 값 사용
        code = getattr(self, "last_login_code", "") or self.ui.code_name.text().strip()

        # 코드가 비어있으면 로그인 화면으로 보내기
        if not code:
    ...
    def verify_login_code_online(self, code: str) -> tuple[bool, str]:
        ...
        try:
            r = requests.post(
                self.LICENSE_URL,
                json={"code": code, "api_key": self.LICENSE_API_KEY, "device": self.device_id},
        ...
        try:
            ...
            # Apps Script가 HTML(로그인 페이지/에러페이지)을 줄 수도 있어서 방어
            try:
    ...
    def load_accounts_json_local(self) -> dict:
        ...
        # ✅ 1) “사용자가 보게 될” 저장 위치 = exe가 있는 폴더(또는 개발 시 main.py 폴더)
        if getattr(sys, "frozen", False):
            base_dir = Path(sys.executable).resolve().parent   # dist\KM_AutoTool\
        else:
            base_dir = Path(__file__).resolve().parent

        user_json = base_dir / "accounts.json"

        # ✅ 2) PyInstaller 번들 내부(_internal / _MEIPASS)에 포함된 기본 accounts.json(있으면)
        bundled_json = None
        ...
        # ✅ 3) user_json이 없으면: (번들에 있으면 복사) → (없으면 default 생성)
    ...
    # 창 크기 (로그인만 작게)
    # -------------------------
    # def apply_login_window_size(self):
    #     # 로그인 화면: 작은 창
    #     self.setFixedSize(900, 650)

    def apply_main_window_size(self):
    ...
    def apply_dark_theme(self):
        ...
        /* ✅ "코드" 라벨을 굵고 크게
           - Qt Designer에서 해당 QLabel의 objectName을 "code"로 꼭 설정해야 적용됨 */
        #code {
            font-size: 24pt;
            font-weight: 800;
        }
